...
export default function DetailModal({
  ...
  // Shared scroll lock
  useBodyScrollLock(open && myLevel === 1);
        ...
        {open && (
          ...
          >
            ...
            <motion.div
              ...
              className={`relative w-full ${widthCls} bg-white dark:bg-[#0f172a] border border-slate-200 dark:border-slate-800 rounded-2xl shadow-2xl flex flex-col max-h-[85vh] overflow-hidden text-slate-800 dark:text-slate-100`}
            >
              {/* Header */}
                ...
                <div className="flex items-center gap-3">
                  ...
                  {icon && (
                    <motion.div 
                      ...
                      className={`p-2 rounded-xl shrink-0 ${iconBg}`}
                    >
                      {icon}
                    </motion.div>
                  )}
              ...
              {/* Scrollable Body with Fade-in and Safe iOS padding */}
              <motion.div 
                initial={{ opacity: 0 }}
                animate={{ opacity: 1 }}
                transition={{ delay: 0.08 }}
                className="flex-1 overflow-y-auto p-6 pb-[max(1.5rem,env(safe-area-inset-bottom))] text-slate-700 dark:text-slate-300"
              >
                {children}
              </motion.div>
            </motion.div>
